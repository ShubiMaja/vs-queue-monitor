...
function applySessionSelectionToGraph() {
  ...
  const findSelectedSession = () => {
    const latest =
      [...queueSessions].reverse().find((s) => s.points && s.points.length) || (queueSessions.length ? queueSessions[queueSessions.length - 1] : null);
    if (wantedKey === "latest") return latest;
    let s = queueSessions.find((x) => x.key === wantedKey) || null;
    if (!s && wantedKey.startsWith("t:")) {
      const m = /^t:(\d+)/.exec(wantedKey);
      const ep = m ? Number.parseInt(m[1], 10) : Number.parseInt(wantedKey.slice(2), 10);
      if (Number.isFinite(ep) && ep > 0) {
        s =
          queueSessions.find((x) => x.startEpoch != null && Math.floor(x.startEpoch) === ep) ||
          queueSessions.find((x) => x.markerEpoch != null && Math.floor(x.markerEpoch) === ep) ||
  ...
  // Prefer the already-indexed sessions (what the dropdown was built from).
  // Rebuilding here can regenerate keys and makes selection feel like it "does nothing."
  let s = findSelectedSession();

  // If we can't find it, try a one-time rebuild from the largest available buffer, then retry.
  if (!s && (sessionsIndexBuffer || logBuffer)) {
    try {
    ...
    s = findSelectedSession();
  }

  graphPoints = s && s.points ? s.points.slice() : [];
  ...
  // Reset derived "live" state so progress/ETA are computed from the selected session.
  connectPhaseStartedEpoch = null;
  progressAtFrontEntry = null;
  leftConnectQueueDetected = lastPosition === 0;
  positionOneReachedAt = null;
  if (graphPoints.length) {
    // If the session reaches the front, capture the first time it did for elapsed/ETA stability.
    for (const [t, p] of graphPoints) {
      if (p <= 1) {
        positionOneReachedAt = t;
        break;
      }
    }
  }
  ...
  try {
    ...
    const t0 = pts.length ? pts[0][0] : null;
    const t1 = pts.length ? pts[pts.length - 1][0] : null;
    const p0 = pts.length ? pts[0][1] : null;
    const p1 = pts.length ? pts[pts.length - 1][1] : null;
    const when = (t) => (t == null ? "—" : formatGraphHoverTime(t));
    appendHistory(
      wantedKey === "latest"
        ? `Session applied: latest · sessions ${queueSessions.length} · points ${pts.length}`
        : s
          ? `Session applied: ${wantedKey}\n- sessions indexed: ${queueSessions.length}\n- points: ${pts.length}\n- start: ${when(t0)} (pos ${p0 == null ? "—" : p0})\n- end: ${when(t1)} (pos ${p1 == null ? "—" : p1})`
          : `Session not found in indexed log window: ${wantedKey}\n- sessions indexed: ${queueSessions.length}\n- tip: pick a bigger log window (large log indexing cap)`,
    );
  } catch {
    // ignore
  }
...
function computeSelectedSessionStats() {
  ...
  const avgMinPerPos = minutes != null && minutes > 0 && cleared != null && cleared > 0 ? minutes / cleared : null;
  return { startPos, endPos, cleared, seconds, minutes, avgMinPerPos };
}
...
function renderSelectedSessionStats() {
  ...
  if (ui.infoStatAvg) ui.infoStatAvg.textContent = stats.avgMinPerPos == null ? "—" : `${stats.avgMinPerPos.toFixed(2)} min/pos`;
}
...
ui.btnCopyStats?.addEventListener("click", async () => {
  ...
  const txt =
    ...
    `Avg Rate: ${stats.avgMinPerPos == null ? "—" : `${stats.avgMinPerPos.toFixed(2)} min/pos`}\n` +
    `\n` +
    `Generated by https://bit.ly/vs-queue-monitor\n`;
  try {
    await navigator.clipboard.writeText(txt);
