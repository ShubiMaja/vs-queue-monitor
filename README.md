...
- **No backend and no local server**: the app is shipped as a **single HTML file** you open directly (or via GitHub Pages). Local development uses `npm run dev` (see **Build** below).
...
| `scripts/build-dist.mjs` | Build script (inlines assets into `dist/`) |
| `assets/` | Icons and sounds (copied into `dist/` by the build) |
...
| `docs/DESIGN.md` | Product & UX design contract |
| `docs/WEB-PLATFORM.md` | Hosting, CI/Pages policy, and engineering reference |
