...
- Shows **Last change**, **Last threshold alert**, **Resolved log path**, and **Global Rate** — average minutes per position over every forward queue step in the **full** graph (all segments); same number as the KPI **All** line under **Rate**.
...
- **Minutes per position** appears under KPI **Rate** as **Last N** (window / dwell model; N = **Prediction window** in Settings) and **All** (full-graph mean). Dwell caps apply to the windowed line. At **position 0** (queue finished), both stay fixed — derived from log timestamps only, not a clock that keeps running after you finish.
