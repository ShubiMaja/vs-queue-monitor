...
- **Header:** Log source row with folder/file pickers; on **narrow widths** the bar **stacks** (title row, then path, then notifications) and the path **truncates** with ellipsis instead of overflowing. **Help** (**?**) lists typical Vintage Story data paths and where the config file is stored.
