# Real-life examples: local web + SSH

Reference catalog for **architecture discussions** (local HTTP + embedded webview or browser, **SSH** workflows). The app is **`python monitor.py`** → Starlette on **127.0.0.1** + **pywebview** (Windows / macOS / Linux desktops), or **`--web-browser`** / **SSH** tunnel. This doc does not describe UI layout in detail.
...
**`python monitor.py`** (same as optional **`--web`**) runs a **local HTTP server on `127.0.0.1`** and **`vs_queue_monitor.engine`**. The UI opens in an **embedded** desktop window where pywebview is supported; otherwise use **`--web-browser`** or forward the port (**`ssh -L`**) and open **`http://127.0.0.1:<port>/`** in a browser on your machine.
