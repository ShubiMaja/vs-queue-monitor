...
class QueueMonitorEngine:
    ...
    def __init__(self, initial_path: str='', auto_start: bool=True) -> None:
        ...
        self.avg_window_var = tk.StringVar(value=str(self.config.get('avg_window_points', DEFAULT_PREDICTION_WINDOW_POINTS)))
        ...
        self.completion_sound_enabled_var = tk.BooleanVar(value=bool(self.config.get('completion_sound_enabled', True)))
        ...
        self.write_history('WARNING — Work in progress; AI-assisted code. Expect bugs and rough edges. Verify log paths, queue readings, and alerts yourself; do not rely on this tool as a sole source of truth.')
        self.write_history("VS Queue Monitor started. Waiting for a path. Parser looks for queue lines like 'Client is in connect queue at position: N'.")
        try:
    ...
    def _handle_interrupted_tail(self, position: Optional[int], queue_sess: int) -> None:
        ...
        if position is not None and queue_sess > self._interrupt_baseline_session and (queue_sess != self._dismissed_new_queue_session):
            if messagebox.askyesno('New queue detected', 'A new queue run was detected in the log.\n\nLoad it? This will reset the graph and threshold alerts for the new run.', parent=self):
