#!/usr/bin/env python3
"""
VS Queue Monitor — terminal UI.

Drives the same ``QueueMonitorApp`` logic as ``monitor.py`` via a hidden Tk root
(periodic ``update()``), so behavior matches the GUI. Requires ``textual``.
"""
...
# Tk must be importable (same as GUI).
try:
    import tkinter as tk
except ImportError as exc:  # pragma: no cover
    print("Tkinter is required (same as the GUI monitor).", file=sys.stderr)
...
def _patch_messagebox_for_tui() -> None:
    """Auto-accept 'new queue run' prompts so the headless pump does not block on Tk dialogs."""
...
class VSQueueTui(App[None]):
    """Textual front-end; embeds hidden ``mon.QueueMonitorApp`` for identical logic."""
    ...
    def __init__(self, initial_path: str = "", auto_start: bool = True) -> None:
        ...
        self._gui: Optional[mon.QueueMonitorApp] = None
    ...
    def on_mount(self) -> None:
        ...
        self._gui = mon.QueueMonitorApp(initial_path=self._initial_path, auto_start=False)
        self._gui.withdraw()
        log = self.query_one("#log", RichLog)
        g = self._gui

        def write_history_tui(self2: mon.QueueMonitorApp, message: str) -> None:
            mon.QueueMonitorApp.write_history(self2, message)
            try:
        ...
        try:
            ...
            if g._queue_progress is not None:
                try:
                    ...
                    prog = 0.0
            text = (
                f"[bold]{mon.APP_DISPLAY_NAME}[/] v{mon.VERSION}  (hidden Tk engine — same logic as GUI)\n\n"
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        ...
        g.source_path_var.set(raw or mon.DEFAULT_PATH)
        try:
            mon.save_config(g.get_config_snapshot())
        except Exception:
            pass
        if g.running:
            g.stop_monitoring()
        g.after(100, g.start_monitoring)
        self._write_log(f"Path set to: {raw or mon.DEFAULT_PATH}")
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="VS Queue Monitor — terminal UI (Textual + same engine as monitor.py)")
    p.add_argument(
        "--path",
        dest="path",
        default="",
        help="Initial Logs folder path (same as GUI --path)",
    )
    p.add_argument(
        "--no-start",
        action="store_true",
        help="Do not auto-start monitoring",
    )
    return p
...
def main() -> int:
    ...
    app = VSQueueTui(initial_path=args.path, auto_start=not args.no_start)
    app.run()
    return 0
